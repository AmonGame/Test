...
{
	SetPlanelInteractable,
	SetPlanelNoInteractable,
	Blend,
}
...
public class EventDispatcher
{
	...
	public static Dictionary<EventNames, List<Action>> emptyEvents;
	public static Dictionary<EventNames, List<Action<object[]>>> argsEvents;
	public static Dictionary<EventNames, List<Action>> tmpEmptyEvents;
	public static Dictionary<EventNames, List<Action<object[]>>> tmpArgsEvents;

	public static void Add(EventNames eName, Action proc)
	{
		if (emptyEvents == null)
		{
			emptyEvents = new Dictionary<EventNames, List<Action>>();
			tmpEmptyEvents = new Dictionary<EventNames, List<Action>>();
		}
	...
	public static void Add(EventNames eName, Action<object[]> proc)
	{
		string en = eName.ToString();

		if (argsEvents == null)
		{
			argsEvents = new Dictionary<EventNames, List<Action<object[]>>>();
			tmpArgsEvents = new Dictionary<EventNames, List<Action<object[]>>>();
		}
	...
	public static void Remove(EventNames eName)
	{
	...
	public static void SendEvent(EventNames eName, params object[] args)
	{
		stack++;
...
//		Debug.Log("stack-- " + stack);
		...
		if (stack == 0)
		{
			...
			{
				foreach (KeyValuePair<EventNames, List<Action>> pair in tmpEmptyEvents)
				{
			...
			{
				foreach (KeyValuePair<EventNames, List<Action<object[]>>> pair in tmpArgsEvents)
				{
