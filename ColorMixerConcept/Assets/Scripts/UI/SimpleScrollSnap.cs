...
public class SimpleScrollSnap : MonoBehaviour, IBeginDragHandler, IEndDragHandler, IDragHandler, IPointerDownHandler, IPointerUpHandler
{
	...
	{
		//imageLoad = GetComponent<ImageLoadScore>();
	...
	public GameObject AddToFront(GameObject panel, bool withSetup = true)
	{
		if (withSetup)
		{
			return Add(panel, 0);
		...
		{
			return AddNoSetup(panel, 0);
		}
	}
	public GameObject AddToBack(GameObject panel, bool withSetup = true)
	{
		if (withSetup)
		{
		...
		{
			return AddNoSetup(panel, NumberOfPanels);
		}
	}

	public GameObject AddNoSetup(GameObject panel, int index)
	{
		...
		if (NumberOfPanels < 3 && infinitelyScroll == true)
		{
			infinitelyScroll = false;
			OnInfiniteScrolling();
		}
		else if(NumberOfPanels >= 3 && infinitelyScroll == false)
		{
			infinitelyScroll = true;
			OnInfiniteScrolling();
		} 
		return panel;
	}
